...
public class Github {
	...
	private VDMSeq getAllRepos() {
		...
		for (Iterator iterator_12 = MapUtil.rng(Utils.copy(accounts)).iterator(); iterator_12.hasNext();) {
	...
	public VDMSeq getTopRepos() {
		...
		for (Long i = (long) l.size(); byVar_1 < 0 ? i >= toVar_2 : i <= toVar_2; i += byVar_1) {
			...
			for (Long j = 1L; j <= toVar_1; j++) {
				if (stargazers(((Repository) Utils.get(sorted_list, j)))
						.size() > stargazers(((Repository) Utils.get(sorted_list, j.longValue() + 1L))).size()) {
					Repository temp = ((Repository) Utils.get(sorted_list, j));
					Utils.mapSeqUpdate(sorted_list, j, ((Repository) Utils.get(sorted_list, j.longValue() + 1L)));
